    ...
    <div class="cart-tool">
      <div class="select-all">
        <input
          ...
          type="checkbox"
          :checked="isAllCheck"
          @click="updateAllIsChecked($event.target.checked)"
        />
        <span>全选</span>
      </div>
      <div class="option">
...
export default {
  ...
  computed: {
    ...
    totalPrice() {
      ...
      this.cartInfoList.forEach((element) => {
        if (element.isChecked == 1) sum += element.cartPrice * element.skuNum;
      });
      return sum;
    },
    isAllCheck() {
  ...
  methods: {
    ...
    deleteCart(skuId) {
      this.$store.dispatch("shopcart/deleteCartList", skuId);
      this.getData();
    },
    async deleteAllCart(skuId) {
      // this.cartInfoList.forEach((element) => {
      //   if (element.isChecked == 1) this.$store.dispatch("shopcart/deleteCartList", element.skuId);
      // });
      // this.getData();
      try {
    ...
    cartIsChecked(cart) {
      let isChecked = 1;
      cart.isChecked == 0 ? (isChecked = 1) : (isChecked = 0);
      this.$store.dispatch("shopcart/cartIsChecked", {
        skuId: cart.skuId,
        isChecked: isChecked,
      });
      this.getData();
    },
    updateAllIsChecked(checked) {
      console.log(checked);
      let isChecked;
      checked ? (isChecked = 1) : (isChecked = 0);
